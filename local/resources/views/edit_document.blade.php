...
@section('content')
  <div class="portlet light">
      ...
      @component('components.alert')
        ...
        @endslot
      @endcomponent
      <form role="form" class="form-horizontal" action="{{route('documents.update',$document->id)}}" enctype="multipart/form-data" method="post">
        {{method_field('PATCH')}}
        {{csrf_field()}}
        <div class="form-body">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <input type="number" name="document_number" value="{{$document->number}}" placeholder="شماره" class="form-control">
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <input type="text" name="document_date" value="{{$document->date}}" placeholder="تاریخ" class="form-control persian_date">
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                  ...
                  <div class="col-md-9">
                    <select class="form-control select2" name="document_categories" id="categories_id">
                      ...
                      @foreach ($categories as $item)
                        @if ($item!=null)
                          <option {{$item->id==$document->category->id?'selected':''}} value="{{$item->id}}">{{$item->name}}</option>
                        @endif
                      @endforeach
                    </select>
                  </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="col-md-9">
                    <input type="number" name="document_page_count" value="{{$document->total_pages}}" placeholder="تعداد صفحات" class="form-control">
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="row">
                  ...
                  <div class="col-md-9">
                    <select class="form-control select2" name="document_department" id="department_id">
                      ...
                      @foreach ($departments as $item)
                        <option {{$item->id==$document->department->id?'selected':''}} value="{{$item->id}}">{{$item->name}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="col-md-9">
                    <input type="text" name="receiver"   value="{{$document->receiver}}" placeholder="مرسل الیه " class="form-control ">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="col-md-9">
                    <textarea name="document_subject" value="" lang="fa" placeholder="موضوع" rows="2" class="form-control">{{$document->subject}}</textarea>
                  </div>
                </div>
              </div>
            <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="col-md-9">
                    ...
                    <textarea name="description" value=""  lang="fa" placeholder="ضمایم" rows="2" class="form-control">{{$document->description}}</textarea>
                  ...
                  <div class="col-md-9">
                    <select class="form-control select2" name="language_id" >
                      ...
                      @foreach ($document_language as $item)
                        <option {{$item->id==$document->document_language->id ?'selected':''}} value="{{$item->id}}">{{$item->language_name}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
            ...
            <div class="col-md-3 col-sm-4 mix category_1 mix_all fancybox" style="display: block;  opacity: 1;">
              <div class="mix-inner">
                ...
                <div class="mix-details">
                  <div class="links">
                    <a class="mix-link" onclick="return confirmDelete()" id="demo_5" href="{{route('delete_document_image',$file->id)}}">
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <div class="col-md-9">
                  ...
                  <a href="{{route('saved_documents')}}" class="btn default">بازگشت</a>
...
function confirmDelete(){
  var check = confirm('آیا مطمئن هستید این فایل را حذف نمایید؟');
  if(check==true){
      return true;
  }
  else{
  ...
  $('#categories_id').change(function() {
    ...
    $(document).ready(function() {
      $('input[type="file"]').change(function () {
        ...
        // get total two sided pages from form
        var total_pages = $("input[name='document_page_count']").val()*2;

        // get selected file count after choosing
        var total_selected_files = $(this)[0].files.length;

        // get uploaded files count from db
        var total_db_files = {{$db_files_count}};

        // if total images i.e. from db and selected is less than total two sided pages then valid
        if( (total_db_files + total_selected_files) > total_pages) {
